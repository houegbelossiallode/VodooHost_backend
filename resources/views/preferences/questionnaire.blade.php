            ...
            <div class="dashboard-content-wrap">
                ...
                <div class="dasboard-widget-title fl-wrap" id="sec1">
                    <h5 class="page-title">
                        ...
                        Questionnaire d’expérience culturelle
                    </h5>
                    <span class="property-title">
                        Profil : <strong>{{ auth()->user()->nom ?? 'Visiteur' }}</strong>
                    </span>
                </div>

                <div class="dashboard-list-box fl-wrap">

                    <form action="{{ route('hoost.preferences.store') }}" method="POST" class="custom-form">
                        @csrf
                        ...
                        <div class="equipments-grid">
                            @php
                                ...
                                // $selected : array des id déjà choisis (si existant)
                                $selected =
                                    $selected ?? (optional(auth()->user()->culturalPreference)->divinites ?? []);
                            @endphp

                            @forelse($divinites as $divinite)
                                @php
                                    $isChecked = in_array($divinite->id, (array) $selected);
                                @endphp
                                ...
                                <div class="equipment-card {{ $isChecked ? 'active' : '' }}">
                                    ...
                                    <input class="equipment-checkbox" type="checkbox" name="divinites[]"
                                        value="{{ $divinite->id }}" id="divinite-{{ $divinite->id }}"
                                        {{ $isChecked ? 'checked' : '' }}>
                                </div>
                            @empty
                        ...
                        <div class="mt-2">
                            ...
                            @php
                                $wantLive = optional(auth()->user()->preferences)->assister_rituel ?? false;
                            @endphp

                            <div class="add-list-media-header">
                                <label class="radio inline">
                                    <input type="radio" name="assister_rituel" value="1" checked>
                                    <span>OUI</span>
                                </label>
                            </div>
                            <div class="add-list-media-header">
                                <label class="radio inline">
                                    <input type="radio" name="assister_rituel" value="0">
                                    <span>NON</span>
                                </label>
                            </div>
                        </div>
