        ...
        <div class="container dasboard-container">
            ...
            <div class="dashboard-content-wrap">
                ...
                <div class="dashboard-list-box fl-wrap">
                    ...
                    <form action="{{ route('hoost.logements.rituels.update', $logement->id) }}" method="POST"
                        class="custom-form">
                        @csrf
                        @method('PUT')
                            ...
                            @forelse($rituels as $rituel)
                                <div class="equipment-card">
                                    <div class="rituel-icon">
                                        <img src="{{ $rituel->symbole }}" alt="{{ $rituel->titre }}"
                                            class="rituel-symbole-img" loading="lazy">
                                    </div>
                                    {{-- <div class="equipment-icon">
                                    <i class="fas {{ $equipement->icon ?? 'fa-check' }}"></i>
                                </div> --}}
                                    <label class="equipment-label" for="equipment-{{ $rituel->id }}">
                                        {{ $rituel->titre }}
                                    </label>

                                    <input class="equipment-checkbox" type="checkbox" name="rituels[]"
                                        value="{{ $rituel->id }}" id="equipment-{{ $rituel->id }}"
                                        {{ in_array($rituel->id, $selected) ? 'checked' : '' }}>
