@extends('layouts.app')

@section('section')
<div class="dashboard-content">
    <div class="dashboard-menu-btn color-bg"><span><i class="fas fa-bars"></i></span>Dashboard Menu</div>
    <div class="container dasboard-container">

        <!-- Titre -->
        <div class="dashboard-title fl-wrap">
            <div class="dashboard-title-item"><span>Pays</span></div>
            @include('partials/hearder2')
        </div>
        <!-- Titre end -->

        <div class="dasboard-wrapper fl-wrap">
            <div class="dasboard-widget-title fl-wrap" style="display:flex;justify-content:space-between;align-items:center;">
                <h5>Liste des pays</h5>
                <a href="{{ route('hoost.pays.create') }}"
                    class="btn color-bg float-btn">
                    <i class="fas fa-plus"></i> Ajouter un pays
                </a>
            </div>

            <div class="dasboard-widget-box fl-wrap">
                <div class="row">
                    @forelse ($pays as $pay)
                        

                        <div class="col-md-6">
                            <div class="bookings-item fl-wrap">
                                <!-- Header -->
                                <div class="bookings-item-header fl-wrap">
                                    
                                    <h4>
                                        <a href="#">
                                            {{ ucfirst($pay->libelle) }}
                                        </a>
                                    </h4>
                                </div>

                                <!-- Contenu -->
                                <div class="bookings-item-content fl-wrap">
                                    <ul>
                                        <li>
                                            <strong>Créé le :</strong>
                                            <span>{{ optional($pay->created_at)->format('d/m/Y H:i') }}</span>
                                        </li>
                                    </ul>
                                </div>

                                <!-- Footer -->
                                <div class="bookings-item-footer fl-wrap">
                                    <span class="message-date">
                                        {{ optional($pay->updated_at)->format('d/m/Y H:i') }}
                                    </span>
                                    <ul>
                                        <li>
                                            <a href="{{ route('hoost.pays.edit', $pay->id) }}"
                                               class="tolt" data-microtip-position="top-left" data-tooltip="Éditer">
                                                <i class="far fa-edit"></i>
                                            </a>
                                        </li>
                                        <li>
                                            <form id="delete-pay-{{ $pay->id }}"
                                                  action="{{ route('hoost.pays.destroy', $pay->id) }}"
                                                  method="POST" style="display:none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                            <a href="#" class="tolt"
                                               data-microtip-position="top-left" data-tooltip="Supprimer"
                                               onclick="event.preventDefault();
                                                        if(confirm('Supprimer ce pay ?'))
                                                        document.getElementById('delete-pay-{{ $pay->id }}').submit();">
                                                <i class="far fa-trash"></i>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    @empty
                        <div class="col-12">
                            <p>Aucun pays enregistré pour le moment.</p>
                        </div>
                    @endforelse
                </div>
            </div>

            
        </div>
    </div>
</div>

{{-- <style>
/* Image du rituel : pas de débordement, recadrée */
.bookings-item-header img.rituel-cover{
    width:100%;
    height:180px;           /* ajuste 160–220px selon ton design */
    object-fit:cover;
    border-radius:8px;
    display:block;
}

/* Titre sous l’image */
.bookings-item-header h4{
    margin-top:10px;
    font-size:16px;
    font-weight:600;
}

/* Sécurise le header contre tout dépassement */
.bookings-item-header{
    position:relative;
    overflow:hidden;
}
</style> --}}
@endsection
