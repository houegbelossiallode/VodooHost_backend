        ...
        <div class="container dasboard-container">
            ...
            <div class="dashboard-title fl-wrap">
                <div class="dashboard-title-item">
                    <span>Affectation des points forts</span>
                </div>
            ...
            <div class="dashboard-content-wrap">
                ...
                <div class="dasboard-widget-title fl-wrap" id="sec1">
                    <h5 class="page-title">
                        ...
                        Affectation des points forts
                    </h5>
                    <span class="property-title">
                        Quartier : <strong>{{ $quartier->libelle }}</strong>
                    </span>
                </div>

                <div class="dashboard-list-box fl-wrap">
                    <form action="{{ route('hoost.quartiers.points.update', $quartier->id) }}" method="POST" class="custom-form">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="equipments-grid">
                            @forelse($points as $point)
                                <div class="equipment-card">
                                    <input class="equipment-checkbox" type="checkbox" name="points[]"
                                        value="{{ $point->id }}" id="equipment-{{ $point->id }}"
                                        {{ in_array($point->id, $selected) ? 'checked' : '' }}>

                                    {{-- <div class="equipment-icon">
                                    <i class="fas {{ $point->icon ?? 'fa-check' }}"></i>
                                </div> --}}

                                    <label class="equipment-label" for="equipment-{{ $point->id }}">
                                        {{ $point->titre }}
                                    </label>
                                </div>
                            @empty
